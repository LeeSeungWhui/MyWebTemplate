...
const switchPropList = [
    { name: 'dataObj/dataKey?', description: 'EasyObj boolean 또는 Y/1 계열 값을 스위치 상태와 연결' },
    { name: 'checked/defaultChecked?', description: '외부에서 제어하는 상태 또는 초기 ON/OFF 값' },
    { name: 'onChange/onValueChange?', description: '토글 후 boolean 값과 연결 정보 전달' },
    ...
    <DocSection
      ...
      title="11. 스위치 (Switch)" description={
        <div className="space-y-4 rounded-xl bg-slate-50/80 p-5 text-sm text-slate-700 ring-1 ring-slate-900/5">
          <p>Switch는 설정, 권한, 알림처럼 즉시 켜고 끄는 boolean 입력입니다. <code>role="switch"</code>와 <code>aria-checked</code>를 포함하며 EasyObj 데이터 연결과 외부 상태 제어를 모두 지원합니다.</p>
          ...
          <p className="text-xs text-slate-500">연결된 값은 <code>true</code>, <code>Y</code>, <code>1</code> 계열을 ON으로 해석해 서버·폼 데이터와 함께 쓰기 쉽습니다.</p>
        </div>
      }
    >
