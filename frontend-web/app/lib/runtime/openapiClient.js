...
async function loadOpenApiClient() {
  const spec = await apiJSON('/openapi.json', { method: 'GET' }, { authless: true })
 ...
 * 갱신일: 2026-01-18
 */
export async function openapiRequest(operationId, parameters = null, data = null, config = {}) {
  const api = await getOpenApiClient()
  const op = api.getOperation?.(operationId)
  if (!op) {
    throw new Error(`Unknown OpenAPI operationId: ${String(operationId || '')}`)
  }
  const axiosConfig = api.getAxiosConfigForOperation(operationId, [parameters, data, config])
  ...
  return apiRequest(url, { method, headers, body: axiosConfig?.data }, { authless })
}
 ...
 * 갱신일: 2026-01-18
 */
export async function openapiJSON(operationId, parameters = null, data = null, config = {}) {
  const api = await getOpenApiClient()
  const op = api.getOperation?.(operationId)
  if (!op) {
    throw new Error(`Unknown OpenAPI operationId: ${String(operationId || '')}`)
  ...
  const headers = axiosConfig?.headers || {}
  const authless = !!axiosConfig?.authless
  return apiJSON(url, { method, headers, body: axiosConfig?.data }, { authless })
}
