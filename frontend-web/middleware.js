...
export async function middleware(req) {
  ...
  if (path.startsWith('/login')) {
    ...
    if (nextParam || reasonParam) {
      ...
      if (nextParam) {
        res.cookies.set(NX_COOKIE, sanitizeInternalPath(nextParam), { path: '/', httpOnly: true, sameSite: 'lax', maxAge: 300 })
      }
      const safeReason = sanitizeBase64Url(reasonParam)
      if (safeReason) {
        res.cookies.set(AUTH_REASON_COOKIE, safeReason, { path: '/', httpOnly: true, sameSite: 'lax', maxAge: 60 })
      }
      return res
    }
    // refresh_token만 있고 access_token이 없는/만료된 상태면, 서버에서 access 재발급 후 목적지로 보내준다.
    if (hasAuthCookie) {
      const res = NextResponse.redirect(new URL('/api/session/bootstrap', req.url))
  ...
  if (hasAuthCookie && !hasValidAccessToken) {
    ...
    res.cookies.set(NX_COOKIE, nextValue, { path: '/', httpOnly: true, sameSite: 'lax', maxAge: 300 })
    if (req.cookies.get(AUTH_REASON_COOKIE)) {
      res.cookies.set(AUTH_REASON_COOKIE, '', { path: '/', maxAge: 0 })
    }
    return res
