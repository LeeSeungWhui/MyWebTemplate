...
- 예시(`app/dashboard`): `initData.jsx`에서 `PAGE_CONFIG.MODE/API`를 선언하고, `page.jsx`는 서버 초기 로더(`buildDashboardInitialData` 또는 `loadServerPageData`)로 SSR/CSR 분기를 일관 처리한다.
- 검증: `frontend-web/__tests__` 대시보드 데이터 로더 테스트에서 MODE 분기, 엔드포인트 누락, SSR 성공/실패 케이스를 단위 테스트로 고정했다.
