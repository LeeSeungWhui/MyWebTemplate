...
- 공개 페이지 공통 GNB를 통해 `/sample/*` 기반 탐색을 통일한다(로그인 메뉴 비노출).

### Scope
- 포함
  - `/` 랜딩(히어로, 서비스 카드, 스크린샷, 스택 뱃지, CTA, 푸터)
  - 공개 GNB(샘플 드롭다운 포함) + 모바일 햄버거/드로어
  - 공개 메뉴에서 인증 동선 제거(템플릿 라우트는 직접 URL 접근만 허용)
  - 루트 공개 전환 및 인증 사용자 리다이렉트 정책 연동
- 제외
  - 샘플 상세 기능 구현(`/sample/*` 내부 동작)

### Interface
- 라우트(UI)
  - `GET /` (공개)
- GNB 링크
  - `샘플`(드롭다운): `/sample`, `/sample/dashboard`, `/sample/crud`, `/sample/form`, `/sample/admin`
  - `컴포넌트`: `/component`
  - `포트폴리오`: `/sample/portfolio`
...
- Hero
  ...
  - CTA: `샘플 체험하기`(solid), `컴포넌트 보기`(outline)
...
- Footer
  - 배경: `bg-gray-900`
  - 구성: 로고 + 카피라이트 + 링크(샘플 허브/컴포넌트/포트폴리오)

### Data & Rules
- 랜딩 CTA는 최소 2개(`샘플 체험하기`, `컴포넌트 보기`)를 제공한다.
- 공개 GNB는 공개 페이지에서 동일 UX를 유지한다(sticky + backdrop blur).
- 루트 공개 정책은 미들웨어 규칙과 충돌 없이 동작해야 한다.
- 샘플 드롭다운은 `샘플 홈`, `샘플 대시보드`, `CRUD 관리`, `복합 폼`, `관리자 화면` 항목을 제공한다.
- 공개 GNB는 `/`, `/sample/*`에서 동일한 메뉴 구조를 유지한다.
- 로그인/회원가입/비밀번호찾기 링크는 공개 GNB/푸터에서 노출하지 않는다.
- 스크린샷 갤러리 이미지는 초기 구현 시 placeholder를 사용하고, 샘플 페이지 완성 후 실제 캡처 이미지로 교체한다.
- i18n 규칙
  - 랜딩/GNB/푸터의 사용자 노출 문구(헤드라인/카드명/버튼/메뉴)는 `lang.ko.js` 키로 관리한다.
...
- AC-4: 샘플 스크린샷 갤러리 카드 3개 클릭 시 각각 `/sample/dashboard`, `/sample/form`, `/sample/admin`으로 이동한다.
- AC-5: 기술 스택 뱃지 6개(`Next.js 15`, `React 19`, `Python`, `FastAPI`, `SQLAlchemy`, `Vitest`)가 노출된다.
- AC-6: 하단 CTA 섹션에 버튼 `샘플 보기`가 노출되고 `/sample` 링크가 동작한다.
- AC-7: 공개 GNB에 `샘플/컴포넌트/포트폴리오` 메뉴가 노출되고 `로그인` 메뉴는 노출되지 않는다.
- AC-8: 모바일 뷰포트에서 햄버거 메뉴를 열면 GNB와 동일한 링크 집합이 표시된다.
- AC-9: 인증 사용자의 `/` 접근은 `/dashboard` 또는 정책상 지정된 템플릿 경로로 전환된다.
- AC-10: 공개 GNB 메뉴 클릭 시 컴포넌트/포트폴리오는 `/component`, `/sample/portfolio`로 이동한다.
- AC-11: 랜딩/GNB의 사용자 노출 문구는 `lang.ko.js` 기반으로 렌더링되고 하드코딩 문자열이 없다.

### Tasks
- T1: `app/(public)` 계열 랜딩 페이지 구조 및 섹션 컴포넌트 작성.
- T2: 공개 GNB 컴포넌트와 샘플 드롭다운 작성.
- T3: 미들웨어/publicRoutes 정책에서 루트 공개 동작과 `/sample/*` 노출 정책 검증.
- T4: 랜딩 섹션/공개 GNB/푸터의 텍스트 키를 `lang.ko.js`로 분리하고 적용.
