...
- 문법 무결성 유지:
  - JS/JSX 파서 기준 구문 오류(식 중간 주석 오염, 미완성 토큰 등) 금지.
- 지양할 것:
...
- 분리한 모듈은 “데이터/스펙”만 담고, 화면 이벤트/비즈니스 처리 로직은 `view.jsx`에 남긴다.
...
- `useEffect` 블록에도 함수/컴포넌트와 동일하게 JSDoc 스타일 동작 설명을 붙인다.
  - 최소 요건: `@description` 또는 `설명:` 1개 이상
