...
- 함수형 컴포넌트 우선(웹/앱)
- 주석/문구는 가능한 한 한글로 작성하되, 변수명/함수명/라이브러리 고유 용어/프로토콜 값 등 코드 식별자는 영어 원문을 유지한다.
- 파일 헤더 주석 필수(모든 런타임 코드 파일; 파일명, 작성자, 갱신일, 설명)
- 함수 헤더 주석 필수(설명, 갱신일)
- import 블록 무결성 유지(모듈 상단에서 import를 모아 선언, 실행문 이후 정적 import 재등장 금지)
...
- 접근성 텍스트(aria-label 등)는 번역 키 관리를 원칙으로 하되, 매우 짧은 보조 문구는 예외적으로 하드코딩을 허용한다.
