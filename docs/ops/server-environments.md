...
Web: `frontend-web/config.ini` → `[APP].backendHost = http://localhost:2000` (프론트는 `/api/bff/api/v1/*`로 호출)
...
Web	frontend-web/config.ini(APP.backendHost)	http://localhost:2000	https://stage.example.com	https://api.example.com
...
릴리스 전 최소선(로컬): lint → pytest(로그인/헬스/트랜잭션) → openapi-validate → web 빌드 스모크
...
시크릿: .env는 로컬에서만, 스테이징/프로덕션은 환경변수 주입
