...
- 프론트는 `initData / page / view` 패턴과 `usePageData` 흐름을 우선 존중한다.
- 백엔드는 기존 Control / DAO / SQL mapper 패턴을 유지한다.
...
직접 확인이 안 되면 그 사유를 남기고 `figma/` 추출물을 근거로 사용한다.
...
- 프론트/백엔드 코드 변경 후: `cvfit-workflow`
- Unit 구현/수정: `cvfit-workflow`
- "면밀 검수/전체 검수/P0 검수" 요청: `cvfit-qa-suite`
- 이력서 변환/최적화 업로드 검수 요청: `cvfit-qa-optimize-upload`
- 새 이력서 만들기 검수 요청: `cvfit-qa-new-resume`
- 내 이력서 목록/상세/수정 검수 요청: `cvfit-qa-my-resume`
- 리뷰/검토/코드리뷰 요청: `scripts/cli/review-with-gemini.sh`
- 특정 QA 플로우 점검: 해당 QA 스킬
...
- 커밋이 필요할 때는 기능 단위로 묶고, 한글 메시지로 작성한다.
